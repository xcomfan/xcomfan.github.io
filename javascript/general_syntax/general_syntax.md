---
layout: page
title: "General Syntax"
permalink: /javascript/general_syntax
---

[General Information]({% link javascript/general_syntax/general_information.md %})

[Code Comments]({% link javascript/general_syntax/comments.md %})

[Variables]({% link javascript/general_syntax/variables.md %})

[Data Types]({% link javascript/general_syntax/data_types/data_types.md %})

[Functions]({% link javascript/general_syntax/functions.md %})

[Comparison Operators]({% link javascript/general_syntax/comparison.md %})

[Conditionals]({% link javascript/general_syntax/comparison.md %})

[Logical Operators]({% link javascript/general_syntax/logical_operators.md %})

[Loops]({% link javascript/general_syntax/loops.md %})

[Exception Handling]({% link javascript/general_syntax/exception_handling.md %})

[Ternary Operator]({% link javascript/general_syntax/ternary.md %})

[ES6 importing and exporting code blocks]({% link javascript/general_syntax/imports.md %})

[Promises]({% link javascript/general_syntax/promises.md %})
